 ...
 * requests.
 */
public class MyTutorSpeechlet implements SpeechletV2 {
	...
	private SpeechletResponse checkAnswer(Session session) {
		...
		if (answerFromUser == null) {
			...
			List<Object> answerFromUserTransformed = new ArrayList<Object>();
			...
			System.out.println("Correct Answer--->" + correctAnswer.toLowerCase());
			...
			if (!answerFromUserTransformed.contains(correctAnswer)) {
				...
				speechText =  MessageBuilder.getAffirmativePhrase(true)+MessageBuilder.CONGRATULATIONS_AUDIO_FILE+ " You got it! Let's try another one. ";
